...
class SourceDiscovery:
    ...
    def candidate_routes(self, source_base: str) -> List[Tuple[str, str, Set[str]]]:
        ...
        if self.cfg.max_routes_per_domain > 0:
    ...
    def resolve_domains(self) -> None:
        limit_label = str(self.cfg.max_domains) if self.cfg.max_domains > 0 else "none"
        self.progress(
            f"Resolving and analyzing domains (usable source limit {limit_label})..."
        )
        ...
        for org in list(self.orgs.values()):
            for raw_url in sorted(org.raw_urls):
                ...
                analyzed_count += 1
                record = self.records.get(source_domain)
                if record and self.article_routes(record):
                    usable_count += 1
                progress_total = (
                    f"{usable_count}/{self.cfg.max_domains}"
                    if self.cfg.max_domains > 0
                    else str(usable_count)
                )
                self.progress(
                    f"Domain progress: {analyzed_count} analyzed; "
                    f"{progress_total} usable domains found."
                )
                if (
                    self.cfg.max_domains > 0
                    and usable_count >= self.cfg.max_domains
                ):
                    self.progress(
                        "Reached --max-domains usable source limit; "
                        "stopping domain analysis."
                    )
                    return
        if self.cfg.max_domains > 0 and usable_count < self.cfg.max_domains:
            self.progress(
                f"Requested {self.cfg.max_domains} usable sources, but exhausted "
                f"all candidate domains after finding {usable_count}."
            )
