...
def grouped_route_contexts(
    data: dict[str, Any],
) -> list[tuple[str, deque[RouteContext]]]:
    grouped: dict[str, deque[RouteContext]] = {}
    for ctx in iter_route_contexts(data):
        grouped.setdefault(ctx.source_domain, deque()).append(ctx)
    return list(grouped.items())


def latest_item_rows(ctx: RouteContext) -> Iterable[dict[str, Any]]:
    for item in ctx.route.get("latest_items", []):
        normalized = normalize_url(item.get("url", ""), ctx.route_url)
        if not normalized:
            continue
        yield {
            "url": normalized,
            ...
            "discovered_published_at": item.get("published_at") or None,
            "discovered_at": now_iso(),
            "discovery_method": "source_discovery_latest_item",
        }


def discover_diverse_urls(
    data: dict[str, Any],
    limit: int | None,
    timeout: int,
    delay: float,
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    sources = grouped_route_contexts(data)
    pending: dict[str, deque[dict[str, Any]]] = {
        source_domain: deque() for source_domain, _ in sources
    }
    routes_by_source = dict(sources)
    for source_domain, routes in sources:
        for ctx in routes:
            pending[source_domain].extend(latest_item_rows(ctx))
    seen: set[str] = set()
    rows: list[dict[str, Any]] = []
    errors: list[dict[str, Any]] = []

    while routes_by_source and (limit is None or len(rows) < limit):
        made_progress = False
        for source_domain in list(routes_by_source):
            if limit is not None and len(rows) >= limit:
                break

            queue = pending[source_domain]
            routes = routes_by_source[source_domain]

            while not queue and routes:
                ctx = routes.popleft()
                for item in discover_route(ctx, timeout):
                    if "_error" in item:
                        errors.append(
                            {
                                "route_url": ctx.route_url,
            ...
            if not queue and not routes:
                del routes_by_source[source_domain]
                del pending[source_domain]

        if not made_progress and not any(pending.values()):
            break
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Maximum number of unique article URLs to write.",
    )
    ...
    rows, errors = discover_diverse_urls(
        data,
        limit=args.limit,
    ...
    print(f"Wrote {len(rows)} article URLs to {output_path}")
